...
function indekseGoreFenomen(fenomenler,a) {
  
  
  const b = fenomenler[a].profile
  return a + ". indekste bulunan fenomen: " + b
}
...
function fenomenGonderimSayisi(fenomenler,name){

  for(let i =0 ; i<fenomenler.length;i++){
  if(fenomenler[i].profile == name){
...
function platformaGoreCokGonderiYapanFenomen(fenomenler,platform){
  ...
  let result;
  for(let i =0;i<fenomenler.length;i++){
    if(fenomenler[i].platform== platform && fenomenler[i].posts>maxPosts){
      maxPosts=fenomenler[i].posts;
      result=fenomenler[i].profile;
    }
  }
  return result
}
